            ...
            <div class="panel panel-info">
                ...
                <div class="panel-body">
                    <table class="table table-bordered">
                        <thead>
                        <tr>
                            <th colspan="9" class="text-center">診斷報告</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                        ...
                        <c:if test="${bundle.getTotal() > 0}">
                            <c:forEach var="count" begin="0" end="${bundle.getTotal()-1}">
                                <tr class="text-center">
                                    <td>${dateformater.format(bundle.getEntry().get(count).getResource().getDateAsserted())}</td>
                                    <td>${bundle.getEntry().get(count).getResource().getDerivedFromFirstRep().getResource().getName()}</td>
                                    <td>${bundle.getEntry().get(count).getResource().getInformationSource().getResource().getName().get(0).getText()}</td>
